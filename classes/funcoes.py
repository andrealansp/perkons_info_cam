...
def get_snapshot(endereco, nome_arquivo="snapshot.jpg", params=None):
    ...
    """
    try:
        response = requests.get(f"http://{endereco}/cgi-bin/snapshot.cgi", params=params,
                                auth=HTTPDigestAuth("admin", "perkons6340"))
...
def roteadores_offline():
    roteadores = database.selecionar_ip_roteadores()
    roteadores_sem_resposta = []
    try:
        for roteador in roteadores:
            if not verificar_ping(str(roteador[1])):
                 roteadores_sem_resposta.append((roteador[0],roteador[1],f"Ponto {roteador[0]} off-line!"))
